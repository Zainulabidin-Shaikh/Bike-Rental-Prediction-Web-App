...
class DataManager:
    ...
    def initialize_prod_database(self) -> None:
        ...
        # this line saves the DataFrame to a parquet file in the production folder
        # The parquet format is efficient for storage and retrieval, especially for large datasets.
        
        # Ensure the prediction file is ready for new predictions
        # This ensures that the prediction file is clean before starting inference
        # If the prediction file exists from previous runs, it is deleted to avoid appending to old data.
        # This is important to ensure that each inference run starts with a fresh slate for predictions.
        # This prevents any confusion or errors that might arise from mixing old and new predictions.
        # If the prediction file does not exist, it will be created when predictions are saved.
        # This is a common practice in data pipelines to ensure data integrity and consistency.
        # The prediction file is used to store the results of the inference runs.
        # It will be updated with new predictions as the pipeline processes each timestamp.
        ...
        if os.path.exists(prediction_path):
            ...
            # This ensures that we start with a clean slate for predictions
            # If the file does not exist, it will be created when predictions are saved.
            # doing this because we want to ensure that each inference run starts with a fresh slate for predictions.

    @staticmethod # static method indicates that this method does not depend on instance variables
